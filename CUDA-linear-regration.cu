...
__device__ double d_residual_error(double xIntercept, double yIntercept, double slope, double c) {
  double e = (slope * xIntercept) + c - yIntercept;
  return e * e;
}
double rms_error(double slope, double c) {
  ...
  for(i=0; i<n_data; i++) {
    error_sum += residual_error(data[i].xIntercept, data[i].yIntercept, slope, c);  
  }
...
__global__ void d_rms_error(double *slope, double *c,double *error_sum_arr,point_t *d_data) {
  int i = threadIdx.x + blockIdx.x *blockDim.x;
error_sum_arr[i] = d_residual_error(d_data[i].xIntercept,d_data[i].yIntercept, *slope, *c);
}
